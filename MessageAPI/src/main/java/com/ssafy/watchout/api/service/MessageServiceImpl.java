...
public class MessageServiceImpl implements MessageService {
	...
	@Override
	public void sendMMSMessage(String accessToken, MMSMessageRequestDto requestDto) throws IOException {
		String authValue = Base64.getEncoder()
		...
		Builder requestBodyBuilder = new MultipartBody.Builder().setType(MultipartBody.FORM)
				...
				.addFormDataPart("image_cnt", requestDto.getFiles().size()+"") // 이미지 첨부 개수를 입력해 주세요. (최대 3개까지 첨부 가능합니다.)
				;
		for (int i = 0; i < requestDto.getFiles().size(); i++) {
			MultipartFile file = requestDto.getFiles().get(i);
			requestBodyBuilder.addFormDataPart("images"+i, file.getName(), RequestBody.create(new File(file.getOriginalFilename()),
					MultipartBody.FORM));
		}
